...
// Name: Alesandra Brittis-Tannenbaum
...
public class BackendGraph<T>  {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            for (T temp : copyPath.dataSequence) {
                dataSequence.add(temp);
            }
        ...
        public int compareTo(Path other) {
            int cmp = (int) (this.distance - other.distance);
            if (cmp != 0)
    ...
    protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
        ...
        List<T> visited = new ArrayList<T>(); // visited paths
        frontier.add(shortestPath); // adds path to frontier
        // if there is no path found from start to end
        // if(!vertices.containsKey(startVertex) || !vertices.containsKey(endVertex)){
        // throw new NoSuchElementException();
        // }
        while (!frontier.isEmpty()) {
            Path currPath = frontier.poll(); // take the path at the beginning of the queue
            visited.add(currPath.end.data);
            if (currPath.end.data.equals(end)) { // at the last node in the path
                shortestPath = currPath; // set shortest path to the current path
                return shortestPath; // reached the end
            } else {
                for (Edge e : currPath.end.edgesLeaving) {
                    Path otherPath = new Path(currPath, e);
                    if (!visited.contains(otherPath.end)) {
                        frontier.add(otherPath); // add other path to the queue
                    }
                }
            }
