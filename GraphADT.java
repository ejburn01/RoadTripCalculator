...
public class GraphADT<T>{
   ...
   * ordered list
   * of the data within each node on this path, and also the distance or cost of
   * all edges
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    if (start == null || end == null)
      throw new NoSuchElementException("Cannot find shortest path between null data");

    // Check for no such vertex.
    if (!vertices.containsKey(start) || !vertices.containsKey(end))
      throw new NoSuchElementException("No such vertex");

    // Create a priority queue of frontier paths.
    PriorityQueue<Path> frontier = new PriorityQueue<>();

    // Create a priorty queue of known paths from start-to-end.
    PriorityQueue<Path> known = new PriorityQueue<>();

    // Create a path for the start vertex.
    ...
    while (!frontier.isEmpty()) {
      ...
      // Remove the path with the smallest distance from the queue.
      Path workingPath = frontier.poll();

      // Grab the working end vertex.
      Vertex workingEndVertex = workingPath.end;

      if (workingEndVertex.data.equals(end)) {
        known.add(workingPath);
        continue;
      }

      for (Edge e : workingPath.end.edgesLeaving) {

        // Prevent loops by checking if the edge is already in the path.
        if (workingPath.dataSequence.contains(e.target.data))
          continue;

        Path workingNewPath = new Path(workingPath, e);
        frontier.add(workingNewPath);
      }
    }

    // If our known priority queue is empty then throw a NoSuchElementException.
    if (known.isEmpty()) {
  ...
  /**
   * Uses Dijkstra's shortest path algorithm to find the shortest paths between a
   * list of nodes.
   * 
   * @param nodes A list of nodes to find the shortest path for.
   * @return the shortest path from start to end, as computed by Dijkstra's
   *         algorithm
   * @throws NoSuchElementException when no path from start to end can be found,
   *                                including when no vertex containing start or
   *                                end can be found
   */
  protected Path dijkstrasShortestPath(List<T> nodes) {
    ...
    // Create a path for the start vertex.
    Path startPath = new Path(vertices.get(nodes.get(0)));
    ...
    while (!frontier.isEmpty()) {
      ...
      // If the end vertex is in out path, add it to the known list.
      if (workingEndVertex.data.equals(nodes.get(nodes.size() - 1))
          && workingPath.dataSequence.size() == nodes.size()) {
        known.add(workingPath);
        continue;
      }

      // Temporarily hold the minimum edge.
      Edge workingMinimumEdge = new Edge(null, Double.POSITIVE_INFINITY);
      for (Edge e : workingPath.end.edgesLeaving) {

        // Check if the edge found is the target and whether or not we are at the end of
        // the path.
        if (e.target.data.equals(nodes.get(nodes.size() - 1)) && workingPath.dataSequence.size() == nodes.size() - 1) {
          workingMinimumEdge = e;
          break;
        }

        // If the end vertex is found, skip it.
        if (e.target.data.equals(nodes.get(nodes.size() - 1))) {
          continue;
        }

        // Check if the edge is already in the path.
        if (workingPath.dataSequence.contains(e.target.data)) {
          continue;
        }

        // Check if the edge is the minimum edge.
        if (e.weight < workingMinimumEdge.weight) {
          workingMinimumEdge = e;
        }
      }

      // Add the minimum edge to the working path.
      Path workingNewPath = new Path(workingPath, workingMinimumEdge);
    ...
    {
      throw new NoSuchElementException("No path found from " + nodes.get(0) + " to " + nodes.get(nodes.size() - 1));
    }
  ...
  /**
   * Returns the shortest path in a list of nodes.
   * Uses Dijkstra's shortest path algorithm to find the shortest path.
   * 
   * @param nodes The data set to find the shortest path for.
   * @return list of data item in vertices in order on the shortest path between
   *         vertex
   *         with data item start and vertex with data item end, including both
   *         start and end and all nodes in the list.
   * @throws NoSuchElementException when no path from start to end can be found
   *                                including when no vertex containing start or
   *                                end can be found
   */
  public List<T> shortestPath(List<T> nodes) {
    return dijkstrasShortestPath(nodes).dataSequence;
  }
  ...
  public double getPathCost(T start, T end) {
    System.out.println(dijkstrasShortestPath(start, end).distance);
